...
## 1. Miejsce klasyfikacji w potoku parsowania

Główna pętla parsowania w `LegalDocumentParser` przekazuje kolejne akapity OpenXml
do `ParserOrchestrator.ProcessParagraph()`. Tam, zanim cokolwiek zostanie zbudowane,
każdy akapit przechodzi przez klasyfikator.

```
LegalDocumentParser.Parse()
    └─► ParserOrchestrator.ProcessParagraph(paragraph, context)
            ├─ GetFullText() + Sanitize()       ← przygotowanie tekstu
            ├─ StyleId()                         ← bezpieczne pobranie stylu
            ├─ Classify(text, styleId, context)  ← KLASYFIKACJA ◄
            ├─ UpdateArticleContext()             ← kontekst dla AI
            ├─ HandleAmendmentFlow()             ← obsługa nowelizacji
            └─ StructureProcessor.Process()      ← budowanie encji
```

Punkt wejścia klasyfikatora zależy od trybu pracy:

| Tryb | Klasyfikator | Konstruktor orchestratora |
|------|-------------|--------------------------|
| Legacy (domyślny) | `ParagraphClassifier` | `new ParserOrchestrator()` |
| Warstwowy | `LayeredParagraphClassifier` | `new ParserOrchestrator(classifier)` |

---

## 2. Krok 0 — Przygotowanie danych wejściowych

Przed wywołaniem klasyfikatora orchestrator przygotowuje dwa wejścia:
...
`GetFullText()` (z `ParagraphExtensions`) obsługuje specjalne elementy OpenXml:

| Element OpenXml | Zachowanie |
|----------------|-----------|
| Normalny `Run` | Konkatenacja tekstu |
| `SymbolChar` kod `F02D` (font Symbol) | Zastępowanie przez `–` (U+2013 en-dash) |
| `FootnoteReference` | Pomijanie |
| `TabChar` | Dodawanie `\t` |
| `VerticalTextAlignment.Superscript` | Owijanie w `[ ]` |

### 2.2 Identyfikator stylu

```csharp
var styleId = paragraph.StyleId();
// Implementacja: paragraph.ParagraphProperties?.ParagraphStyleId?.Val?.ToString()
```

Użycie operatora `?.` gwarantuje, że brak `ParagraphProperties` nie powoduje wyjątku.
Gdy styl nie jest ustawiony, `styleId` jest `null`.

### 2.3 Kontekst artykułu (tylko tryb warstwowy z AI)

```csharp
// ParserOrchestrator.Classify():
...
`context.CurrentArticleTexts` to lista tekstów bieżącego artykułu. Jest czyszczona
przy każdym nowym artykule i uzupełniana po każdym nienowelizacyjnym akapicie.
Dostarczana do warstwy AI jako kontekst semantyczny.
